...
import com.example.administrator.zxg.util.Glide.GlideCircleTransform;
import com.example.administrator.zxg.util.Glide.GlideRoundTransform;
...
public class GlideImageUtils {
    ...
    public static void loadImageViewDrawable(Context mContext, int path, ImageView mImageView) {
        Glide.with(mContext).load(path).into(mImageView);
    }
    //加载网络图片（圆形）
    public static void loadCircleImage(Context context,String url,ImageView imageView){

        Glide.with(context).load(url).placeholder(R.mipmap.icon_error).error(R.mipmap.icon_error).transform(new GlideCircleTransform(context)).into(imageView);
    }
    //加载网络图片（圆形File）
    public static void loadCircleImageFile(Context context, File url, ImageView imageView){
    ...
    public static void loadRoundCornerImage(Context context,String url,ImageView imageView){
        ...
        Glide.with(context).load(url).placeholder(R.mipmap.icon_error).error(R.mipmap.icon_error).transform(new GlideRoundTransform(context,10)).into(imageView);
    }
